...
while true; do
  ...
  for i in $(seq 0 14); do
    ...
    mosquitto_pub -h mqtt-broker -t lab/temp/$site -m "$temp"
    mosquitto_pub -h mqtt-broker -t lab/humidity/$site -m "$hum"
    mosquitto_pub -h mqtt-broker -t lab/power/$site -m "$power"
    mosquitto_pub -h mqtt-broker -t lab/location/$site -m "$geo"
  done

  # Publish retained discovery JSON for Nagios LLD
  ...
  mosquitto_pub -r -h mqtt-broker -t lab/discovery -m "$discovery"
